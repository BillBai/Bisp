...
enum { LVAL_FUN, LVAL_NUM, LVAL_ERR, LVAL_SYM, LVAL_STR, LVAL_SEXPR, LVAL_QEXPR };
...
struct lval {
    ...
    // if lbuiltin is not NULL, then it is a builtin func, otherwise it is a lambda
    lbuiltin builtin;
    lenv* env;
    lval* formals;
    lval* body;
...
lenv* lenv_copy(lenv* e)
{
    ...
    for (int i = 0; i < e->count; i++) {
        n->syms[i] = (char*)malloc(strlen(e->syms[i]) + i);
...
{
    char* escape = (char*)malloc(strlen(v->str) + 1);
    strcpy(escape, v->str);

    escape = (char*)mpcf_escape(escape);
...
lval* builtin_var(lenv* e, lval* a, char* func)
{
    ...
    for (int i = 0; i < syms->count; i++) {
        LASSERT(a, (syms->cell[i]->type == LVAL_SYM),
                ...
                Got %s, Expected %s", func, ltype_name(syms->cell[i]->type), ltype_name(LVAL_SYM));
    }

    LASSERT(a, (syms->count == a->count - 1),
            "Function %s cannot passed too many arguments for symbols. \
...
lval* lval_call(lenv* e, lval* f, lval* a)
{
    ...
    if (f->formals->count > 0 && strcmp(f->formals->cell[0]->sym, "&") == 0) {
        if (f->formals->count != 2) {
            return lval_err("Function format invalid. \
                    Symbol '&' not folloewed by single symbol.");
        }
